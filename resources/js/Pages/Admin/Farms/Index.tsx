            ...
            header={
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-pine-500 mt-1">{t('farms.subtitle')}</p>
                    </div>
                    <div>
                        <span className="text-sm font-medium text-pine-600 bg-pine-50 px-3 py-1 rounded-full border border-pine-100">{stats.total} {t('farms.farms_total')}</span>
                    </div>
                </div>
            }
        >
                ...
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-white rounded-3xl p-6 shadow-card border border-sand-200">
                            <h3 className="text-sm font-medium text-pine-500">{t('farms.total_farms')}</h3>
                            <p className="mt-2 text-3xl font-bold text-pine-800">{stats.total}</p>
                        </div>
                        <div className="bg-white rounded-3xl p-6 shadow-card border border-sand-200">
                            <h3 className="text-sm font-medium text-pine-500">{t('farms.active_farms')}</h3>
                            <p className="mt-2 text-3xl font-bold text-green-600">{stats.active}</p>
                        </div>
                        <div className="bg-white rounded-3xl p-6 shadow-card border border-sand-200">
                            <h3 className="text-sm font-medium text-pine-500">{t('farms.pending_approval')}</h3>
                    ...
                    <div className="bg-white rounded-3xl shadow-card border border-sand-200 overflow-hidden">
                        {farms.data.length === 0 ? (
                            <div className="p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
                                <h3 className="text-2xl font-bold text-pine-800 mb-2">{t('farms.no_farms')}</h3>
                                <p className="text-pine-500 max-w-md mx-auto mb-8">
                                    {t('farms.no_farms_description')}
                                </p>
                            </div>
                        ) : (
