        ...
        <AppShellLayout>
            ...
            <div className="relative w-full max-w-md bg-bg flex flex-col" style={{ height: '100dvh' }}>
                <div className="flex-1 overflow-y-auto no-scrollbar" style={{ paddingBottom: '88px' }}>
                    ...
                    <div className="bg-card px-6 pt-4 pb-2">
                        <button onClick={() => window.history.back()} className="inline-flex items-center text-sm text-textSecondary hover:text-primary transition-colors">
                    ...
                    <FinancialErrorBoundary context="investment-purchase-configure">
                        <div className="bg-card px-6 pb-6 pt-2">
                            <h2 className="text-xl font-bold text-text mb-4 flex items-center gap-2">
                                <IconTree className="w-6 h-6 text-primary" />
                                {t('invest_in_tree', { identifier: tree.identifier })}
                            </h2>

                            {flash?.error && (
                                <div className="mb-4 bg-danger-50 border border-danger-200 text-danger-700 px-4 py-3 rounded-xl text-sm" role="alert">
                                    {flash.error}
                                </div>
                            )}

                             {flash?.success && (
                                <div className="mb-4 bg-success-50 border border-success-200 text-success-700 px-4 py-3 rounded-xl text-sm" role="alert">
                                    {flash.success}
                                </div>
                            )}

                            {/* Tree Details Card */}
                            <div className="bg-bg rounded-xl p-4 mb-6 border border-border space-y-3">
                                <div className="flex justify-between text-sm">
                                    <span className="text-textSecondary">{t('farm')}</span>
                                    <span className="font-semibold text-text">{tree.farm.name}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-textSecondary">{t('fruit_type')}</span>
                                    <span className="font-semibold text-text">{tree.fruit_crop.fruit_type} ({tree.fruit_crop.variant})</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-textSecondary">{t('price_per_tree')}</span>
                                    <span className="font-semibold text-primary">{formatRupiah(tree.price_idr)}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-textSecondary">{t('expected_roi', { roi: '' }).replace('%', '').trim()}</span>
                                    <span className="font-semibold text-primary">{tree.expected_roi}%</span>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="flex items-center justify-center gap-6">
                                        <button
                                            ...
                                            className="w-12 h-12 rounded-full bg-card border border-border shadow-card flex items-center justify-center text-textSecondary hover:border-primary hover:text-primary disabled:opacity-40 disabled:cursor-not-allowed transition-all active:scale-95 text-xl font-bold"
                                        >
                                            −
                                        </button>
                                        ...
                                        <div className="text-center w-24">
                                            <input
                                                ...
                                                className="w-full text-center text-3xl font-bold text-text border-none focus:ring-0 bg-transparent p-0"
                                            />
                                            <p className="text-[10px] text-textSecondary mt-1 uppercase tracking-wide font-medium">Trees</p>
                                        </div>
                                    ...
                                    {errors.quantity && <p className="mt-2 text-sm text-danger text-center">{errors.quantity}</p>}
                                </div>

                                {/* Total Price Preview */}
                                <div className="bg-primary text-white p-5 rounded-2xl mb-8 shadow-floating">
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="text-primary-100 text-sm font-medium">{t('total_investment', { defaultValue: 'Total Investment' })}</span>
                                        <IconDollar className="w-5 h-5 text-primary-200" />
                                    </div>
                                    <div className="text-3xl font-bold mb-2">
                                        {formatRupiah(totalIdr)}
                                    </div>
                                    <div className="text-primary-100 text-xs flex justify-between">
                                ...
                                <div className="space-y-4 mb-8">
                                    <label className="flex items-start gap-3 p-3 rounded-xl border border-border bg-bg cursor-pointer hover:bg-card hover:border-primary-200 transition-colors">
                                        ...
                                        <span className="text-sm text-textSecondary">
                                            ...
                                            <button type="button" onClick={() => setShowRiskModal(true)} className="text-primary font-semibold hover:underline">
                                                Risk Disclosure
                                            </button>
                                        </span>
                                    </label>
                                    ...
                                    <label className="flex items-start gap-3 p-3 rounded-xl border border-border bg-bg cursor-pointer hover:bg-card hover:border-primary-200 transition-colors">
                                        ...
                                        <span className="text-sm text-textSecondary">
                                            I agree to the{' '}
                                            <a href="/legal/terms" target="_blank" className="text-primary font-semibold hover:underline" onClick={(e) => e.stopPropagation()}>
                                                Terms & Conditions
                                            </a>
                                        </span>
                                    </label>
                                    ...
                                    <div className="mb-8">
                                        ...
                                        <div className="relative">
                                            <select
                                                ...
                                                className="w-full pl-3 pr-10 py-3 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary appearance-none bg-card text-text"
                                            >
                                ...
                                {/* Submit Button */}
                                <PrimaryButton
                                    className="w-full justify-center py-4 text-base rounded-xl shadow-floating mb-4"
                                ...
                                <Link
                                    ...
                                    className="block text-center text-sm font-medium text-textSecondary hover:text-text pb-4"
                                >
                                    {t('cancel_button')}
                                </Link>
                            </form>
            ...
            <Modal show={showRiskModal} onClose={() => setShowRiskModal(false)}>
                <div className="p-6">
                    ...
                    <div className="prose prose-sm max-h-[60vh] overflow-y-auto pr-2 mb-6 text-textSecondary">
