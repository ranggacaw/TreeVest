...
class ReportDataServiceTest extends TestCase
{
    ...
    public function test_get_profit_loss_data_correctly_aggregates_payouts(): void
    {
        $investment = Investment::factory()->create([
            ...
            'amount_idr' => 1000000, // Rp 10,000
        ]);

        Payout::factory()->create([
            'investment_id' => $investment->id,
            'gross_amount_idr' => 500000, // Rp 5,000
            'platform_fee_idr' => 50000, // Rp 500
            'net_amount_idr' => 450000, // Rp 4,500
            'status' => PayoutStatus::Completed,
        ]);

        Payout::factory()->create([
            'investment_id' => $investment->id,
            'gross_amount_idr' => 600000, // Rp 6,000
            'platform_fee_idr' => 60000, // Rp 600
            'net_amount_idr' => 540000, // Rp 5,400
            'status' => PayoutStatus::Completed,
        ]);
        ...
        $this->assertEquals(1000000, $data['rows'][0]['amountInvestedIdr']);
        $this->assertEquals(990000, $data['rows'][0]['totalPayoutsIdr']);
        $this->assertEquals(-10000, $data['rows'][0]['netIdr']);
        $this->assertEquals(-1.0, $data['rows'][0]['actualRoiPercent']);
        $this->assertEquals(1000000, $data['summary']['totalInvestedIdr']);
        $this->assertEquals(990000, $data['summary']['totalPayoutsIdr']);
        $this->assertEquals(-10000, $data['summary']['netIdr']);
    }
    ...
    public function test_get_profit_loss_data_handles_zero_payouts(): void
    {
        ...
        $this->assertCount(1, $data['rows']);
        $this->assertEquals(0, $data['rows'][0]['totalPayoutsIdr']);
        $this->assertEquals(-1000000, $data['rows'][0]['netIdr']);
    ...
    public function test_get_tax_summary_data_filters_by_year(): void
    {
        ...
        $this->assertEquals(1000000, $data['summary']['totalInvestedIdr']);
        $this->assertEquals(500000, $data['summary']['totalIncomeIdr']);
    }
    ...
    public function test_get_tax_summary_data_calculates_correct_totals(): void
    {
        ...
        $this->assertEquals(4500000, $data['summary']['totalInvestedIdr']);
        $this->assertEquals(750000, $data['summary']['totalIncomeIdr']);
        $this->assertEquals(-3750000, $data['summary']['netIdr']);
    }
    ...
    public function test_get_performance_data_aggregates_by_month(): void
    {
        ...
        $this->assertArrayHasKey('investedIdr', $data[0]);
        $this->assertArrayHasKey('payoutsIdr', $data[0]);
        $this->assertArrayHasKey('cumulativeIdr', $data[0]);
    }
}
